...
function cleanNewsTitle(value) {
  return text(value).replace(/^\s*\d{4}\s*(?:[./-]|年)\s*\d{1,2}\s*(?:[./-]|月)\s*\d{1,2}\s*(?:日)?(?:[\s　:：｜|・-]+|$)/u, "").trim();
}
...
function renderNewsPage(row, canonical) {
  ...
  const categoryLabels = { exhibition: "展覧会", open_call: "公募", artist_news: "作家動向", museum: "美術館", nihonga_news: "日本画ニュース", award: "受賞", selection: "入選", solo: "個展", graduation: "卒展", university: "大学", gallery: "画廊" };
  const category = categoryLabels[text(row?.category)] || "日本画ニュース";
...
module.exports = { cleanNewsTitle, displayNewsDate, formatNewsDate, newsSlug, renderNewsPage };
