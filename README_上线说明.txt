...
这个版本用于部署成真正的网站：
- 访问者打开网址，使用邮箱注册 / 登录后进入目录。
- 你打开 /admin，用管理员密码进入后台。
- 后台可以添加、删除、修改画家，也可以生成、删除卡密。
...
2. 打开 SQL Editor。
3. 把 seed/supabase-init.sql 里面的内容全部复制进去运行。
   这一步会创建 artists / licenses 两张表，并导入现在的画家和卡密。
...
1. 在 Supabase SQL Editor 执行 `seed/email-auth.sql`。这一步只创建
   `site_users` 和 `site_sessions` 两张认证表，不会修改现有画家或卡密数据。
...
访问者：
- 打开 Vercel 给你的网址。
- 首次使用选择“注册”，填写邮箱和密码；之后用邮箱登录即可查看画家目录。
- 旧卡密接口仍保留给已部署的兼容页面，切换到邮箱版后不需要把卡密发给普通用户。

你自己：
- 打开 网址/admin。
- 输入 ADMIN_PASSWORD。
- 可以添加画家、修改学校/地区/标签、删除画家、生成或删除卡密。
...
- 生成新卡密发给对方；
- 或删除旧卡密；
- 或把旧卡密状态改为 disabled（这个版本的界面先支持删除，新版本可以继续加“一键停用/延期”按钮）。
...
邮箱登录与原卡密登录可以并行保留，不会删除 licenses 表，也不会影响 /admin 的管理员密码。无需新增 npm 依赖；密码使用 Node.js 内置 crypto.scrypt 哈希。默认长期登录为 30 天，可在 Vercel 添加 AUTH_SESSION_DAYS（1-365）修改；未勾选“记住登录”时使用浏览器会话 Cookie，服务端最长保留 1 天。
...
api/
  云端接口。负责验证卡密、读画家、后台管理。

public/
  网站页面。index.html 是目录页，admin.html 是后台页。

seed/
  初始数据。supabase-init.sql 是最重要的导入文件。
